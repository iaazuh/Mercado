  ...
  return (
    <div className='flex item-center justify-between py-3 px-20 font-medium'>
      ...
      <div className='flex item-center gap-6'>
        <img onClick={()=>setShowSearch(true)} className='w-auto h-5 pl-2 cursor-pointer' src={assets.search_icon} alt="" />        
        <div className='group relative'>
          <Link to={'/login'}><img className='w-auto h-5 cursor-pointer' src={assets.profile_icon} alt="" /></Link>
        ...
        <NavLink to='/cart' className='relative  pr-5'>
          <img className='w-8 min-w-8' src={assets.cart_icon} alt="" />
        ...
        <img onClick={()=>setVisible(true)} className='w-auto h-6 cursor-pointer sm:hidden pr-5' src={assets.menu_icon} alt="" />
      </div>
      {/* sidebar */}
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible? 'w-full' : 'w-0'} `}>
        <div className='flex flex-col text-gray-600'>
          <div onClick={()=> setVisible(false)} className='flex item-center gap-4 p-3 cursor-pointer '>
            <img className='h-4 pt-1' src={assets.arrow} alt="" />
